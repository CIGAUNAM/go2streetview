...
class panoSnapShot():
    ...
    def __init__(self,parentInstance):
       ...
       # Save reference to QWebView with Streetview application
        ...
        self.feat_id = parentInstance.feat_id
        self.actualPOV = parentInstance.actualPOV
    ...
    def sessionDirectory(self):
        ...
        sDir = os.path.join(self.path,'pano_snapshots')
    ...
    def saveSnapShot(self, actualPOV, feat_id, heading):
        self.pov = actualPOV
        self.feat_id = feat_id
        self.pov['heading'] = heading

        core.QgsMessageLog.logMessage("POV: " + str(self.pov), tag="go2streetview", level=core.Qgis.Info)
    ...
    def saveImg(self,path = None):
        urlimg="http://maps.googleapis.com/maps/api/streetview?size=640x640&location="+str(self.pov['lat'])+","+str(self.pov['lon'])+"&heading="+str(self.pov['heading'])+"&pitch="+str(self.pov['pitch'])+"&sensor=false&key="+self.parent.APIkey
        #print urlimg
        if path:
            self.file_name = path
        else:
            ...
            self.file_name = os.path.join(self.sessionDirectory(), str(self.feat_id) + '.jpg')
        core.QgsMessageLog.logMessage("FILENAME: " + self.file_name, tag="go2streetview", level=core.Qgis.Info)
        u = urlopen(urlimg)
